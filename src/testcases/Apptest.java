...
public void selectButton () {
		...
		WebElement allstatic = driver.findElement(By.id("dropdown-class-example"));

        
        Select staticdropdown = new Select(allstatic);

       
        List<WebElement> options = staticdropdown.getOptions();

        int randomstaticclick = new Random().nextInt(options.size() ) ;

        staticdropdown.selectByIndex(randomstaticclick);
